...
export const QUESTION_TEMPLATES = [
  ...
  (entry) => `During focused review, connect this ${entry.topic} cue to the safest response: ${entry.q} What is the best answer?`,
  (entry) => `A learner is practicing questions about ${entry.topic}. Clinical cue: ${entry.q} Which choice is most appropriate?`,
  (entry, subject, difficulty) => `For a ${difficulty} knowledge-check item, use this ${entry.topic} stem: ${entry.q} Which response is best?`,
  ...
  (entry, subject) => `A clinical judgment item gives this ${entry.topic} cue: ${entry.q} Which nursing response is the safest?`,
...
export const ANSWER_REMINDERS = [
  (entry) => `Review focus: connect ${entry.topic} to the safest nursing priority.`,
  ...
  (entry) => `Review note: this concept is meant to become easier with repeated practice.`
];
