...
export function AuthScreen(props) {
  ...
  function renderPasswordInput({ value, onChange, placeholder, visible, setVisible }) {
    return (
      <div style={{ position: "relative" }}>
        <input value={value} onChange={onChange} placeholder={placeholder} type={visible ? "text" : "password"} style={{ width: "100%", padding: "12px 46px 12px 14px", borderRadius: 14, border: `1px solid ${C.border}`, background: "#FBFAF7", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #F5F2EA 0%, #F8F6F1 100%)", padding: width < 640 ? 16 : 24, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      <div style={{ width: "min(1100px, 100%)", display: "grid", gridTemplateColumns: stacked ? "1fr" : "minmax(0, 1.1fr) minmax(340px, 440px)", gap: 20, alignItems: "stretch" }}>
        <div style={{ background: "linear-gradient(145deg, #112240 0%, #16305C 70%, #214778 100%)", borderRadius: 28, padding: stacked ? 24 : 34, color: "#fff", minHeight: stacked ? 420 : 560, display: "flex", flexDirection: "column", justifyContent: "space-between", boxShadow: "0 24px 50px rgba(15, 23, 42, 0.16)", position: "relative", overflow: "hidden" }}>
          ...
          <div>
            ...
            <div style={{ marginTop: 18, fontSize: 15, lineHeight: 1.85, color: "rgba(233,239,247,0.84)", maxWidth: 560 }}>Continue your flashcards, quizzes, uploads, weak-area review, and saved sessions in one supportive workspace built for real learners preparing for demanding exams.</div>
          </div>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 28, padding: 28, boxShadow: "0 18px 36px rgba(15, 23, 42, 0.08)", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <div style={{ display: "flex", gap: 10, marginBottom: 22 }}>
            {[["login", "Sign In"], ["register", "Register"]].map(([value, label]) => (
              <button key={value} type="button" onClick={() => setAuthMode(value)} style={{ flex: 1, padding: "12px 14px", borderRadius: 14, border: authMode === value ? "1px solid rgba(23, 43, 77, 0.12)" : `1px solid ${C.border}`, background: authMode === value ? "linear-gradient(135deg, #1A2740 0%, #24385E 100%)" : "#FBFAF7", color: authMode === value ? "#fff" : C.text, fontWeight: 800, cursor: "pointer" }}>{label}</button>
            ))}
          </div>
          ...
          <div style={{ marginTop: 22, display: "grid", gap: 12 }}>
            {isRegister ? <div><label style={{ fontSize: 12, color: C.muted, fontWeight: 700, display: "block", marginBottom: 6 }}>Name</label><input value={authName} onChange={(event) => setAuthName(event.target.value)} placeholder="Your name" style={{ width: "100%", padding: "12px 14px", borderRadius: 14, border: `1px solid ${C.border}`, background: "#FBFAF7", fontSize: 14, outline: "none", boxSizing: "border-box" }} /></div> : null}
            <div><label style={{ fontSize: 12, color: C.muted, fontWeight: 700, display: "block", marginBottom: 6 }}>Email</label><input value={authEmail} onChange={(event) => setAuthEmail(event.target.value)} placeholder="name@example.com" type="email" style={{ width: "100%", padding: "12px 14px", borderRadius: 14, border: `1px solid ${C.border}`, background: "#FBFAF7", fontSize: 14, outline: "none", boxSizing: "border-box" }} /></div>
            <div>
          ...
          {authNotice ? <div style={{ marginTop: 16, padding: "16px 16px 14px", borderRadius: 16, background: "#F4FBF7", border: `1px solid ${C.accentMid}`, color: C.text }}><div style={{ fontSize: 12, color: C.accent, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em" }}>{authNotice.title}</div><div style={{ marginTop: 8, fontSize: 13, lineHeight: 1.7 }}>{authNotice.body}</div><button type="button" onClick={onDismissNotice} style={{ marginTop: 12, padding: "9px 12px", borderRadius: 10, border: `1px solid ${C.accentMid}`, background: "#FFFFFF", color: C.accent, fontWeight: 700, cursor: "pointer" }}>{authNotice.actionLabel || "Continue"}</button></div> : null}
